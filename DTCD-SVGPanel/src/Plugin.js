...
export class SVGPanel extends PanelPlugin {
  ...
  processDataSourceEvent(eventData) {
    ...
    if (this.#config.isSettingsFromDatasource) {
      if (data.length > 0) {
        ...
        if (data[0]?.adaptiveImg) {
          this.setVueComponentPropValue('adaptiveImg', data[0]?.adaptiveImg);
        }
      }
    }
  ...
  getPluginConfig() {
    const config = {
      ...
      adaptiveImg: this.#vueComponent.adaptiveImg,
    };
    return config;
  }
  ...
  getFormSettings() {
    return {
      fields: [
        ...
        {
          component: 'switch',
          propName: 'adaptiveImg',
          attrs: {
            label: 'Адаптировать картинку по ширине',
            checked: true,
          },
        },
        {
